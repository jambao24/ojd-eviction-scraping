...
FF_0522 <- read.csv("G:/Shared drives/ojdevictions/ScrapeData/full_scrape_20220522/flat_file.csv")
FF_0522_1 = FF_0522[,-1]
FF_0522_1 <- subset(FF_0522_1, select = -c(case_name:status))
FF_0522_1 <- subset(FF_0522_1, select = -c(plaintiff_name:Agent))
FF_0522_1 <- subset(FF_0522_1, select = -c(tenant_lawyer:landlord_lawyer))
FF_0522_1 <- subset(FF_0522_1, select = -c(landlord_has_lawyer:zip))
#View(FF_0522_1)

FF_0522_2 = FF_0522[,-1]
FF_0522_2 <- subset(FF_0522_2, select = -c(case_name:defendant_addr))
FF_0522_2 <- subset(FF_0522_2, select = -c(Judgment_General:tenant_lawyer))
FF_0522_2 <- subset(FF_0522_2, select = -c(FTA:FTAFirst))
FF_0522_2 <- subset(FF_0522_2, select = -c(FTAFirstXJudgmentGeneral:zip))
#View(FF_0522_2)

FF_0522_3 = FF_0522[,-1]
FF_0522_3 <- subset(FF_0522_3, select = -c(case_name:status))
FF_0522_3 <- subset(FF_0522_3, select = -c(plaintiff_name:zip))
#View(FF_0522_3)

# for Goal 3, we want col C (case_code), F (decision), G (judgment date)
judgments_0522 <- read.csv("G:/Shared drives/ojdevictions/ScrapeData/full_scrape_20220522/judgments.csv")
# https://www.youtube.com/watch?v=6inAoQddaj0
# convert date column to Date data type
judgments_0522$date <- as.Date(judgments_0522$date, format = '%m/%d/%Y')
# remove unwanted rows
judgments_0522 = judgments_0522[,-1]
judgments_0522 = judgments_0522[,-1]
judgments_0522 = judgments_0522[,-2]
judgments_0522 = judgments_0522[,-2]
judgments_0522 = judgments_0522[,-5]
#View(judgments_0522)
# filter for rows that contain judgment dates within the current week (2022-05-08 through 2022-05-14)
judgments_0522_curr = judgments_0522[judgments_0522$date > "2022-05-07" & judgments_0522$date < "2022-05-15", ]
judgments_0522_curr <- judgments_0522_curr[complete.cases(judgments_0522_curr),]
...
part1_join <- merge(FF_0522_1, OLC_updated_1, by.x = "case_code", by.y = "Case #")
part2_join <- merge(FF_0522_2, OLC_updated_2, by.x = "case_code", by.y = "Case #")
part3_join <- merge(FF_0522_3, judgments_0522, by = "case_code")
...
part1_join_FTA_match <- sqldf("select * from part1_join_FA_held 
                                                where 
                                                [FTADefault] = '1'
                                                and
                                                [Outcome3] = 'Tenant Default' 
...
part1_join_FTA_match0 <- sqldf("select * from part1_join_FA_held 
                                                where 
                                                [FTA] = '1'
                                                and
                                                [Outcome3] = 'Tenant Default' 
...
# filter through FTA rulings per FF but not OLC
part1_join_FTA_not_match0_1 <- sqldf("select * from part1_join_FA_held 
                                                where 
                                                [FTA] = '1'
                                                and
                                                ([Outcome3] <> 'Tenant Default' or [Outcome3] is null) 
...
# filter through FTA rulings per OLC but not FF
part1_join_FTA_not_match0_2 <- sqldf("select * from part1_join_FA_held
                                                where 
...
# for this we merge the judgments_0522_curr dataframe with the part1_join_evict_match dataframe (confirmed evictions)
this_week_evict_1 <- merge(judgments_0522_curr, part1_join_evict_match, by.x = "case_code", by.y = "case_code")
this_week_evict_2 <- merge(judgments_0522_curr, part1_join_evict_not_match_1, by.x = "case_code", by.y = "case_code")
this_week_evict_3 <- merge(judgments_0522_curr, part1_join_evict_not_match_2, by.x = "case_code", by.y = "case_code")
...
write.table(CountyWeekEvict_curr, file="G:/Shared drives/ojdevictions/FF_vs_OLC_accuracy/CountyWeeklyEvict_0522.csv", row.names=F, sep=",")
...
write.table(part1_join_evict_not_match_1, file="G:/Shared drives/ojdevictions/FF_vs_OLC_accuracy/0522_eviction_false_pos.csv", row.names=F, sep=",")
write.table(part1_join_evict_not_match_2, file="G:/Shared drives/ojdevictions/FF_vs_OLC_accuracy/0522_eviction_false_neg.csv", row.names=F, sep=",")
write.table(part1_join_dismiss_not_match_1, file="G:/Shared drives/ojdevictions/FF_vs_OLC_accuracy/0522_dismissal_false_pos.csv", row.names=F, sep=",")
write.table(part1_join_dismiss_not_match_2, file="G:/Shared drives/ojdevictions/FF_vs_OLC_accuracy/0522_dismissal_false_neg.csv", row.names=F, sep=",")
